...
Для быстрого решения проблемы достаточно первого способа. Этот способ является быстрым. Если хочется решить проблему как следует, то небходимо объединить первый и второй способ. Данные действия могут быть медленней и потребовать больше дискового пространства. Второй способ не имеет смысла без первого. Так же следует помнить, что второй способ удаляет снимки образов диска.
